...
class cylinderEnsemble():
	...
	## Method to fill up cylinder voids within the bounding box with cylinders, assuming all cylinders
	#	have the same radius r. Clearance between cylinders is allowed for through rclearance.
	#
	#@param r
	#@param rclearance
	#
	def fillUpVoids(self,r,rclearance=None):
		...
		if rclearance == None:
			rclearance = r
		for elm in self.lCyl:
			res += pl.sqrt(pl.sqrt((X-elm[0])**2+(Y-elm[1])**2) -(elm[2]+rclearance)+0.j)

		lCylAdd = []
		while res.imag.min() < 1e-6:
	...
	#
	#@see distMatSq
	def removeCollisions(self,fixedR=0.5):
		d = 2*fixedR
		d = d**2	#since the distMatSq is squared
	...
	#@param useAxis	Specify an axis on which to plot the polar plot (defaults to creating a new axis)
	#
	#@return nothing
	#							
	#@see polarPlot
	def polarPlotS(self,S,clim=(0,1),smooth=False,colorbar=True,useAxis=False):
		...
		cax = ax.pcolormesh(self.lphi.ravel(),self.lk,S.T,vmin=clim[0],vmax=clim[1])
		pl.ylim(0,self.lk[-1])
		if colorbar:
			pl.gcf().colorbar(cax)
	...
	def visualise(self,sameColour=False,show=True):
		...
		pl.gca().set(adjustable='box-forced',aspect='equal')
		if sameColour:
			colourwheel = list("b"*len(self.lCyl))
		else:
			colourwheel = list("bgrcmyk"*(len(self.lCyl)//7+1))
		for cyl in self.lCyl:
			circle = pl.Circle( (cyl[0],cyl[1]), cyl[2], color=colourwheel.pop())
